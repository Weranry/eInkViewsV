...
def generate_image(rotate=0, invert=False, tz=None, cmode=None, **kwargs):
    ...
    draw_panel(draw, 16, 60, 180, 'Left', data['detail']['left'], body_font, body_font, img.width, img.height)
    draw_panel(draw, 204, 60, 180, 'Right', data['detail']['right'], body_font, body_font, img.width, img.height)
    draw_panel(draw, 16, 150, 368, 'Note', data['detail']['note'], body_font, body_font, img.width, img.height)

    note = f"{data['summary']['primary']} | {data['summary']['secondary']}"
