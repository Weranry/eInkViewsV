...
from .utils import load_font, draw_safe_text, draw_rows, text_size
...
def generate_image(rotate=0, invert=False, tz=None, cmode=None, **kwargs):
    ...
    draw_safe_text(draw, (16, 10), str(data['meta']['title']), fill=2, font=title_font, img_width=img.width, img_height=img.height)
    draw_safe_text(draw, (16, 44), str(data['meta']['subtitle']), fill=1, font=body_font, img_width=img.width, img_height=img.height)
    ...
    draw_rows(draw, 16, 72, rows, body_font, body_font, img.width, img.height)
