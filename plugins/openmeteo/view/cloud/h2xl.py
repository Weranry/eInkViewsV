...
def generate_image(rotate=0, invert=False, tz=None, cmode=None, **kwargs):
    ...
    f_val = get_font(80, get_root_font_path("font.ttf"))
    f_unit = get_font(32, get_root_font_path("font.ttf"))
    ...
    for i, (cx, cy) in enumerate(centers):
        ...
        draw.text((cx - (bb[2]-bb[0])//2, cy - 64), label, fill=(0, 0, 0), font=f_title)
        ...
        draw.text((start_x, cy - 8), v_str, fill=vcolor, font=f_val)
        draw.text((start_x + (bb_v[2]-bb_v[0]), cy + 32), "%", fill=vcolor, font=f_unit)
