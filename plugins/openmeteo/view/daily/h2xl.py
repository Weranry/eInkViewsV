...
def generate_image(rotate=0, invert=False, tz=None, cmode=None, lat=None, lon=None, **kwargs):
    ...
    # Line
    draw.line([(16, py + 24), (400 - 16, py + 24)], fill=(0,0,0), width=2)
    ...
    for i, d in enumerate(days):
        ...
        draw.text((cx + (col_w - wd_w)/2, py + 40), weekday, font=font16, fill=(0,0,0))
        
        dt_w = draw.textbbox((0,0), date, font=font16)[2] - draw.textbbox((0,0), date, font=font16)[0]
        draw.text((cx + (col_w - dt_w)/2, py + 60), date, font=font16, fill=(0,0,0))
        ...
        draw.text((cx + (col_w - icon_w)/2, py + 96), icon_char, font=font_icon_64, fill=(0,0,0))
        
        # Desc under icon
        desc_w = draw.textbbox((0,0), desc, font=font16)[2] - draw.textbbox((0,0), desc, font=font16)[0]
        draw.text((cx + (col_w - desc_w)/2, py + 176), desc, font=font16, fill=(255,0,0))
        
        # Temp (High / Low)
        hi = f"{d['t_max']}°"
        lo = f"{d['t_min']}°"
        
        hi_w = draw.textbbox((0,0), hi, font=font32)[2] - draw.textbbox((0,0), hi, font=font32)[0]
        draw.text((cx + (col_w - hi_w)/2, py + 220), hi, font=font32, fill=(255,0,0))
        
        lo_w = draw.textbbox((0,0), lo, font=font16)[2] - draw.textbbox((0,0), lo, font=font16)[0]
        draw.text((cx + (col_w - lo_w)/2, py + 256), lo, font=font16, fill=(0,0,0))
        
        if i > 0:
            draw.line([(cx, py + 40), (cx, 300 - 16)], fill=(0,0,0), width=1)
