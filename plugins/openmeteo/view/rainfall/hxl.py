...
def generate_image(rotate=0, invert=False, tz=None, cmode=None, **kwargs):
    ...
    summary_y = sep_y + 6
    summary_text = f"24h {total_precip:.1f}mm  峰值{max_precip:.1f}mm"
    draw_safe_text(draw, (16, summary_y), summary_text, fill=1, font=body_font, img_width=W, img_height=H)

    chart_left = 40
    chart_right = W - 16
    chart_top = summary_y + 22
    chart_bottom = H - 32
    ...
    for i in range(0, n, step):
        ...
        draw_safe_text(draw, (label_x, chart_bottom + 2), hour_label, fill=1, font=label_font, img_width=W, img_height=H)
