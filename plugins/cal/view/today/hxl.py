...
def generate_image(rotate=0, invert=False, cmode=None, **kwargs):
    ...
    cy += gz_th + 2
    shenxiao_line = "{}年".format(shengxiao_str)
    if jieqi_str:
        shenxiao_line += " · {}".format(jieqi_str)
    draw_safe_text(draw, (cx, cy), shenxiao_line, fill=1, font=body_font, img_width=W, img_height=H)
    _, sx_th = text_size(draw, shenxiao_line, body_font)

    cy += sx_th + 2
    festivals = data["festivals"] + data["other_festivals"]
    if festivals:
        fest_str = " · ".join(festivals[:3])
